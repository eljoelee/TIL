...
1. ssh 접속 방법
    ...
    - 퍼블릭 키 확인 : vi /home/<user>/.ssh/authorized_keys
...
4. IP 주소 확인 : nslookup
    - 도메인 질의 절차
        1. /etc/hosts에 IP 주소 확인
        2. DNS Cache에 IP 주소 확인
        3. 확인되지 않는다면 DNS Server에 쿼리 질의
            - Root : .com(TLD) 서버 정보 전달
            - TLD : google.com(SLD) 서버 정보 전달
            - SLD : IP 주소 정보 전달
5. 웹/DB 서버 연결 확인 : telnet [IP/Domain] [Port]
    - TCP/23으로 동작
6. 특정 포트 상태 확인 : netstat -lntp
7. 특정 프로세스 : ps -ef | grep ""
8. 시스템 정보
    - top : 메모리, CPU 사용량 등 확인
        - load average
            - 실행 중인 프로세스 + I/O 작업 대기 프로세스 수를 
             1/5/15분 단위로 표현
        - tasks : 프로세스 개수
            - sleeping : 대기 프로세스 수
            - zombie : 부모가 죽은 자식 프로세스 수
        - Cpu(s) : CPU 사용률
            - hi : 하드웨어 인터럽트에 사용되는 CPU 비율
            - si : 소프트웨어 인터럽트에 사용되는 CPU 비율
        - MiB : 메모리 사용률
            - buff/cache : buffered I/O 메모리 + 캐시 메모리 사용률
        - PR : 스케줄링 우선순위
        - VIRT : 전체 가상 메모리 양(Swap + RAM)
        - RES : 프로세스가 RAM에서 사용 중인 실제 메모리 양
